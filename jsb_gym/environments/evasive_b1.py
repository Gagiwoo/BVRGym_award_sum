    ...
    def update_states(self):
        def safe(value, name):
            if not np.isfinite(value):
                print(f"[⚠️] Invalid value in {name}, replacing with 0.0")
                return 0.0
            return value

        if self.conf.states['update_states_type'] == 1:
            for key in self.aim_block:
                self.f16.state_block[key][0,0] = safe(self.get_distance_to_firing_position(self.f16, self.aim_block[key], scale=True), 'distance')
                self.f16.state_block[key][0,1] = safe(self.get_time_since_missile_active(self.f16, scale=True), 'missile_time')
                a1, a2 = self.get_angle_to_firing_position(self.f16, self.aim_block[key], scale=True)
                self.f16.state_block[key][0,2] = safe(a1, 'angle_sin')
                self.f16.state_block[key][0,3] = safe(a2, 'angle_cos')
                self.f16.state_block[key][0,4] = safe(self.get_velocity(scale=True), 'velocity')
                self.f16.state_block[key][0,5] = safe(self.get_altitude(scale=True), 'altitude')
                self.f16.state_block[key][0,6] = safe(self.get_psi(scale=True), 'psi')
                self.f16.state_block[key][0,7] = safe(self.get_aim_vel0(self.aim_block[key], scale=True), 'aim_vel0')
                self.f16.state_block[key][0,8] = safe(self.get_aim_alt0(self.aim_block[key], scale=True), 'aim_alt0')
                self.f16.state_block[key][0,9] = safe(self.get_v_down(scale=True), 'v_down')
                self.states_extra[key] = self.affp

        elif self.conf.states['update_states_type'] == 2:
            for key in self.aim_block:
                n, e, d = self.get_relative_unit_position_NED(self.f16, self.aim_block[key], scale=True)
                self.f16.state_block[key][0,0] = safe(n, 'rel_north')
                self.f16.state_block[key][0,1] = safe(e, 'rel_east')
                self.f16.state_block[key][0,2] = safe(d, 'rel_down')

                self.f16.state_block[key][0,3] = safe(self.get_time_since_missile_active(self.f16, scale=True), 'missile_time')
                self.f16.state_block[key][0,4] = safe(self.get_altitude(scale=True), 'altitude')
                self.f16.state_block[key][0,5] = safe(self.get_aim_alt0(self.aim_block[key], scale=True), 'aim_alt0')

                vn, ve, vd = self.get_relative_velocity_NED(self.f16, self.aim_block[key], scale=True)
                self.f16.state_block[key][0,6] = safe(vn, 'rel_v_north')
                self.f16.state_block[key][0,7] = safe(ve, 'rel_v_east')
                self.f16.state_block[key][0,8] = safe(vd, 'rel_v_down')

        elif self.conf.states['update_states_type'] == 3:
            for key in self.aim_block:
                dist = safe(self.get_distance_to_firing_position(self.f16, self.aim_block[key], scale=True), 'distance')
                a1, a2 = self.get_angle_to_firing_position(self.f16, self.aim_block[key], scale=True)
                sin_psi = safe(np.sin(np.radians(self.get_psi(False))), 'sin_psi')
                cos_psi = safe(np.cos(np.radians(self.get_psi(False))), 'cos_psi')
                alt = safe(self.get_altitude(scale=True), 'altitude')

                self.f16.state_block[key][0,0] = dist
                self.f16.state_block[key][0,1] = safe(a1, 'angle_sin')
                self.f16.state_block[key][0,2] = safe(a2, 'angle_cos')
                self.f16.state_block[key][0,3] = alt
                self.f16.state_block[key][0,4] = sin_psi
                self.f16.state_block[key][0,5] = cos_psi
